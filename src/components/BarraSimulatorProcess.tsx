...
const BarraSimulatorProcess = () => {
    //se agrego "algoritmo al contexto"
    const { procesos, memoriaTotal, algoritmo } = useProcesoContext();
    ...
    const actualizarColas = (procesosBase: ProcesoCorriendo[], ejecutados: ProcesoCorriendo[]) => {
        ...
        let noEjecutados = procesosBase.filter(
            p => !ejecutados.some(e => e.PID === p.PID)
        );


        // Ordena según algoritmo
        if (algoritmo === "FCFS") {
            // ya integrado
        } else if (algoritmo === "SJF") {
            noEjecutados = [...noEjecutados].sort((a, b) => a.Duration - b.Duration);
